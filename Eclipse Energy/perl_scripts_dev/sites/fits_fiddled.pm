...
#       for example. This looks pretty safe, but any changes will need
...
#
		...
		for my $row (75 .. 79) {
		    ...
		    if ($value) {
			print "$date -  " . $tech[$row] . " : " . $value->unformatted() . "\n" if $DEBUG;
			if($date ne "0000-00-01"){
			    push @data, [$date, $tech[$row], $value->unformatted()] unless $DEBUG;
			}
